...
def shepp_logan_3d(size_out=128, phantom_type="yu-ye-wang", get_ellipsoids=False):
    ...
    for ellipsoid in ellipsoids:
        ...
        phi1 = (
            ellipsoid[6] * np.pi / 180
        )  # 1st Euler angle in radians (rotation about z-axis)
        phi2 = (
            ellipsoid[7] * np.pi / 180
        )  # 2nd Euler angle in radians (rotation about x'-axis)
        phi3 = (
            ellipsoid[8] * np.pi / 180
        )  # 3rd Euler angle in radians (rotation about z"-axis)
...
def _parse_inputs(size_out, phantom_type):
    ...
    if type(size_out) == int:
        ...
        nVoxel = [
            size_out.reshape(-1)[0],
            size_out.reshape(-1)[1],
            size_out.reshape(-1)[2],
        ]
    else:
        nVoxel = [128, 128, 128]
...
if __name__ == "__main__":
    ...
    phantom0, e = shepp_logan_3d(
        size_out=[nVoxelZ, nVoxelY, nVoxelX],
        phantom_type="kak-slaney",
        get_ellipsoids=True,
    )
    phantom1 = shepp_logan_3d(
        size_out=[nVoxelZ, nVoxelY, nVoxelX], phantom_type="yu-ye-wang"
    )
    phantom2 = shepp_logan_3d(
        size_out=[nVoxelZ, nVoxelY, nVoxelX], phantom_type="toft-schabel"
    )
    plt.imshow(
        np.concatenate(
            [
